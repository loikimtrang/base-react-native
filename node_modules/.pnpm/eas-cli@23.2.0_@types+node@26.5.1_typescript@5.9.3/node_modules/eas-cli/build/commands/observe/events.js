...
class ObserveEvents extends EasCommand_1.default {
    ...
    static flags = {
        ...
        'session-id': core_1.Flags.string({
            description: 'Filter by session ID. When no event name is given, lists the events in the session instead of the event-name summary.',
        }),
    ...
    async runAsync() {
        ...
        if (!args.eventName && !flags['all-events'] && !flags['session-id']) {
            const { names, isTruncated } = await (0, planGating_1.withObservePlanGateHandlingAsync)(() => ObserveQuery_1.ObserveQuery.customEventNamesAsync(graphqlClient, {
                appId: projectId,
                startTime,
                endTime,
                platform,
        ...
        const { events, pageInfo } = await (0, planGating_1.withObservePlanGateHandlingAsync)(() => (0, fetchCustomEvents_1.fetchObserveCustomEventsAsync)(graphqlClient, projectId, {
