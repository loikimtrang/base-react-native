...
class FingerprintCompare extends EasCommand_1.default {
    ...
    async runAsync() {
        ...
        if (unavailableSourceHashes.length > 0) {
            ...
            log_1.default.warn(`A detailed diff cannot be shown. ${getUnavailableSourceMessage(unavailableSourceHashes)}`);
        }
        else {
...
async function getFingerprintFromUpdateFragmentAsync(updateWithFingerprint) {
    ...
    const { fingerprint, isFingerprintSourceUnavailable } = await getFingerprintFromFingerprintFragmentAsync(updateWithFingerprint.fingerprint);
    return {
        fingerprint,
...
async function getFingerprintInfoFromHashAsync(graphqlClient, projectId, hash) {
    ...
    const { fingerprint, isFingerprintSourceUnavailable } = await getFingerprintFromFingerprintFragmentAsync(fingerprintFragment);
    let platforms;
...
async function getFingerprintInfoFromBuildIdAsync(graphqlClient, buildId) {
    ...
    const { fingerprint, isFingerprintSourceUnavailable } = await getFingerprintFromFingerprintFragmentAsync(buildWithFingerprint.fingerprint);
    return {
        fingerprint,
        isFingerprintSourceUnavailable,
...
async function fetchFingerprintSourceAsync(fingerprintDebugUrl) {
    ...
    const fingerprintSource = (await fingerprintResponse.json());
...
async function getFingerprintFromFingerprintFragmentAsync(fingerprintFragment) {
    ...
    try {
        ...
        log_1.default.debug(`Failed to fetch source for fingerprint hash ${fingerprintFragment.hash}`, error);
        return {
...
function getUnavailableSourceMessage(hashes) {
