...
function displayBandwidthMetric(metric, indent = '  ') {
    ...
    if (metric.overageValue > 0) {
        log_1.default.log(`${indent}${metric.name} (additional usage): ${chalk_1.default.red(formatBytesDisplay(metric.overageValue))} (${(0, usageUtils_1.formatCurrency)(metric.overageCost)})`);
    }
}
function displayMetric(metric, indent = '  ') {
    ...
    log_1.default.log(`${indent}${progressBar} ${color(percentStr)}`);
    if (metric.overageValue > 0) {
        log_1.default.log(`${indent}${metric.name} (additional usage): ${chalk_1.default.red(`${(0, usageUtils_1.formatNumber)(metric.overageValue)} ${metric.unit ?? ''}`)} (${(0, usageUtils_1.formatCurrency)(metric.overageCost)})`);
    }
}
...
function displayUsage(data, usageData) {
    ...
    if (upcomingInvoice?.lineItems && upcomingInvoice.lineItems.length > 0) {
        ...
        if (data.totalOverageCostCents > 0) {
            log_1.default.log(`  Additional usage: ${chalk_1.default.yellow((0, usageUtils_1.formatCurrency)(data.totalOverageCostCents))}`);
            if (data.builds.overageCostCents > 0) {
                log_1.default.log(`    Builds: ${(0, usageUtils_1.formatCurrency)(data.builds.overageCostCents)}`);
            ...
            if (data.updates.overageCostCents > 0) {
                log_1.default.log(`    Updates: ${(0, usageUtils_1.formatCurrency)(data.updates.overageCostCents)}`);
            }
        }
