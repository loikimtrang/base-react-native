...
exports.resolveWorkflowRunSshInput = resolveWorkflowRunSshInput;
...
function resolveWorkflowRunSshInput({ ssh, idleTimeoutSeconds, }) {
...
class WorkflowRun extends EasCommand_1.default {
    ...
    static flags = {
        ...
        'ssh-idle-timeout': core_1.Flags.integer({
            description: 'Seconds to keep an SSH session open after its job finishes, while no client is connected. The session is always available for the whole job. Requires --ssh. Must be between 0 and 3600; defaults to 0 (close as soon as the job finishes and nobody is connected).',
    ...
    async runAsync() {
        ...
        if (sshInput) {
            log_1.default.log('SSH enabled. Each VM job logs the `eas workflow:ssh` command to connect.');
        }
        if (!flags.wait) {
            if (flags.json) {
