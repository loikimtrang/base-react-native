...
exports.isPermanentGraphqlError = isPermanentGraphqlError;
 ...
 */
function isPermanentGraphqlError(error) {
    if (!(error instanceof core_1.CombinedError) || error.networkError) {
        return false;
    }
    return (error.graphQLErrors.length > 0 &&
        error.graphQLErrors.every(e => e?.extensions?.errorType === 'USER' && !e?.extensions?.isTransient));
}
