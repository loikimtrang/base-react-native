...
function handleBuildRequestError(error, platform, accountName) {
    logBuildRequestErrorDebugInfo(error);
    const graphQLErrors = Array.isArray(error?.graphQLErrors)
        ? error.graphQLErrors
        : [];
    const graphQLErrorCode = graphQLErrors[0]?.extensions?.errorCode;
    if (graphQLErrorCode && graphQLErrorCode in SERVER_SIDE_DEFINED_ERRORS) {
        ...
        const isFreeTierLimitError = graphQLErrorCode === 'EAS_BUILD_FREE_TIER_LIMIT_EXCEEDED' ||
            graphQLErrorCode === 'EAS_BUILD_FREE_TIER_IOS_LIMIT_EXCEEDED';
        throw new ErrorClass(isFreeTierLimitError
            ? `${message}\nRun ${(0, plans_1.formatStarterSubscribeCommand)(accountName)} to upgrade to the Starter plan.`
            : message);
    }
        ...
        throw new Error(`Build request failed. Make sure you are using the latest eas-cli version. If the problem persists, report the issue.${errorDetails}`);
    }
    throw error;
}
function formatBuildRequestErrorDetails(error, graphQLErrors) {
    ...
    const graphQLRequestIds = new Set(graphQLErrors.map(graphQLError => graphQLError?.extensions?.requestId));
...
function getResponseRequestId(response) {
    return (response?.headers?.get?.('expo-request-id') ?? response?.headers?.get?.('x-request-id') ?? null);
}
function logBuildRequestErrorDebugInfo(error) {
    const debugInfo = collectBuildRequestErrorDebugInfo(error);
    if (debugInfo) {
        log_1.default.debug(`Build request error details:\n${JSON.stringify(debugInfo, null, 2)}`);
    }
}
function collectBuildRequestErrorDebugInfo(error) {
...
function collectErrorDebugInfo(error) {
    const debugInfo = {};
...
function collectResponseDebugInfo(response) {
    if (!response) {
        return null;
    }
    const debugInfo = {};
