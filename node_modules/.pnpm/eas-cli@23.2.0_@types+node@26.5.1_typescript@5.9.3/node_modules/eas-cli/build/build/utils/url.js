...
function getBuildLogsUrl(build, hash) {
    const { app } = build;
    const url = `/accounts/${app.ownerAccount.name}/projects/${app.slug}/builds/${build.id}${hash ? `#${hash}` : ''}`;
...
function getInternalDistributionInstallUrl(build) {
    if (build.platform === generated_1.AppPlatform.Ios) {
        return `itms-services://?action=download-manifest;url=${(0, api_1.getExpoApiBaseUrl)()}/v2/projects/${build.app.id}/builds/${build.id}/manifest.plist`;
    }
