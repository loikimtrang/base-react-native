...
exports.resolveBetaFeedbackTarget = resolveBetaFeedbackTarget;
/**
 ...
 */
const FEEDBACK_URL_PATTERN = /\/(betaFeedbackCrashSubmissions|betaFeedbackScreenshotSubmissions)\/([^/?#]+)/i;
/**
 ...
 */
function resolveBetaFeedbackTarget({ idOrUrl, type, defaultType, }) {
    const match = idOrUrl.match(FEEDBACK_URL_PATTERN);
    if (match) {
        const urlType = match[1].toLowerCase().startsWith('betafeedbackcrash')
            ? 'crash'
            : 'screenshot';
        if (type && type !== urlType) {
            throw new Error(`--type ${type} does not match the URL, which points at ${urlType} feedback. Omit --type when passing a URL.`);
        }
        return { id: decodeURIComponent(match[2]), type: urlType };
    }
    if (/^[a-z][a-z\d+\-.]*:\/\//i.test(idOrUrl)) {
        throw new Error(`Could not tell whether "${idOrUrl}" points at crash or screenshot feedback. Pass the App Store Connect API URL from \${{ app_store_connect.beta_feedback.url }}, or pass the ID with --type.`);
    }
