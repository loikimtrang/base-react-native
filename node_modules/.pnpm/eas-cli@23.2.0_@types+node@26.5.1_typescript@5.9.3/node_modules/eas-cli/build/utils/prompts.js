...
const defaults_1 = require("../environments/defaults");
...
async function promptVariableEnvironmentAsync({ nonInteractive, selectedEnvironments, multiple = false, canEnterCustomEnvironment = false, graphqlClient, projectId, }) {
    if (nonInteractive) {
        throw new Error(`The \`--environment\` flag must be set when running in \`--non-interactive\` mode. Default environments: ${defaults_1.DEFAULT_ENVIRONMENTS.join(', ')}.`);
    }
    let allEnvironments = defaults_1.DEFAULT_ENVIRONMENTS;
    if (graphqlClient && projectId) {
        const projectEnvironments = await (0, variables_1.getProjectEnvironmentVariableEnvironmentsAsync)(graphqlClient, projectId);
        allEnvironments = [...new Set([...defaults_1.DEFAULT_ENVIRONMENTS, ...projectEnvironments])];
    }
    if (!multiple) {
