...
function displayOverageWarning({ tier, name, hasFreePlan, planValue, limit, overageCount, overageCostCents, }) {
    ...
    const upgradeHint = ` Run ${(0, plans_1.formatStarterSubscribeCommand)(name)} to upgrade to the Starter plan.`;
    if (tier === 'approaching') {
        const percentUsed = calculatePercentUsed(planValue, limit);
    ...
    // tier === 'over' && paid plan
    log_1.default.warn(chalk_1.default.bold(`You've used ${overageCount} build${overageCount === 1 ? '' : 's'} beyond your included credits this billing period (${formatCents(overageCostCents)} in additional usage so far).`));
