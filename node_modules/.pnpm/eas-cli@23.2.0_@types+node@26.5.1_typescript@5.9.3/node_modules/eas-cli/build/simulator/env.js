...
async function resetSimulatorEnvAsync(projectDir, expectedDeviceRunSessionId) {
    const simulatorDotenvFilePath = getSimulatorEnvFilePath(projectDir);
    try {
        const currentEnv = (0, dotenv_1.parse)(await fs.readFile(simulatorDotenvFilePath, 'utf8'));
        if (currentEnv[exports.EAS_SIMULATOR_SESSION_ID] !== expectedDeviceRunSessionId) {
            ...
            log_1.default.debug(`Skipping ${exports.SIMULATOR_DOTENV_FILE_NAME} reset: it belongs to simulator session ${currentEnv[exports.EAS_SIMULATOR_SESSION_ID] ?? '(unknown)'}, not ${expectedDeviceRunSessionId}.`);
            return;
        }
