...
exports.formatMetricCandidateTitle = formatMetricCandidateTitle;
exports.formatLogCandidateTitle = formatLogCandidateTitle;
 ...
 */
function formatMetricCandidateTitle(event) {
    const displayName = (0, metricNames_1.getMetricDisplayName)(event.metricName);
    const value = `${event.metricValue.toFixed(2)}s`;
    const timestamp = (0, formatUtils_1.formatLogTimestamp)(event.timestamp);
 ...
 * candidate picker.
 */
function formatLogCandidateTitle(event) {
    const timestamp = (0, formatUtils_1.formatLogTimestamp)(event.timestamp);
    const severity = event.severityText ?? (event.severityNumber != null ? String(event.severityNumber) : '-');
    const shortSession = shortSessionId(event.sessionId ?? '');
    const device = `${event.deviceOs} ${event.deviceOsVersion}`;
