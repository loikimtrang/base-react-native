...
const projectNotConfiguredError_1 = require("./projectNotConfiguredError");
...
async function fetchOrCreateProjectIDForWriteToConfigWithConfirmationAsync(graphqlClient, projectInfo, options, actor) {
    ...
    const accountNamesWhereUserHasSufficientPublishPermissions = new Set(allAccounts.filter(a => a.viewerUserPermission.role !== generated_1.Role.ViewOnly).map(it => it.name));
    if (options.nonInteractive) {
        throw (0, projectNotConfiguredError_1.getUnconfiguredProjectError)({ actor, accountName });
    }
