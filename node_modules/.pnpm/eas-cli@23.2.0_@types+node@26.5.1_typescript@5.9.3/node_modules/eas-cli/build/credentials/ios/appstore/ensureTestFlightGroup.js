 ...
 */
async function ensureTestFlightGroupExistsAsync(app, { nonInteractive = false } = {}) {
...
async function ensureInternalGroupAsync({ groups, app, nonInteractive, }) {
    ...
    if (!betaGroup.attributes.hasAccessToAllBuilds) {
        if (nonInteractive) {
            ...
            log_1.default.warn(`TestFlight group "${AUTO_GROUP_NAME}" does not have automatic access to new builds. Re-run in interactive mode to regenerate it, or recreate it in App Store Connect.`);
            return betaGroup;
        }
...
async function addAllUsersToInternalGroupAsync(group, users, app) {
    let emails = users
...
async function getTestFlightGroupUrlAsync(group, app) {
