...
class BuildStep extends BuildStepOutputAccessor {
    ...
    constructor(ctx, { id, displayName, inputs, outputs, command, fn, workingDirectory: maybeWorkingDirectory, shell, supportedRuntimePlatforms: maybeSupportedRuntimePlatforms, env, ifCondition, compositeFunctionScope, timeoutMs, __metricsId, __hookId, }) {
    ...
    shouldExecuteStep({ runByDefault }) {
        if (this.compositeFunctionScope &&
            !this.compositeFunctionScope.isActive(jsepEval_1.evaluateIfCondition, runByDefault)) {
            return false;
        }
        if (!this.ifCondition) {
            return runByDefault;
    ...
    evaluateIfCondition(ifCondition, { scope, env, inputs, }) {
        ...
        return (0, jsepEval_1.evaluateIfCondition)(ifCondition, context);
    }
    skip() {
    ...
    interpolateInputsOutputsAndGlobalContextInTemplate(template, inputs) {
        ...
        if (!inputs) {
            ...
            return skipLegacyOutputInterpolation
                ...
                : (0, template_1.interpolateWithOutputs)(interpolatedWithGlobalContext, path => this.getLegacyStepOutputValue(path));
        }
        ...
        const interpolatedWithInputsAndGlobalContext = (0, template_1.interpolateWithInputs)(this.ctx.global.interpolate(template), vars);
        return skipLegacyOutputInterpolation
            ? interpolatedWithInputsAndGlobalContext
            : (0, template_1.interpolateWithOutputs)(interpolatedWithInputsAndGlobalContext, path => this.getLegacyStepOutputValue(path));
    }
    getLegacyStepOutputValue(path) {
    ...
    getInterpolatedEnvOverrides() {
        ...
        const ownEnv = Object.fromEntries(Object.entries(ownOverrides).map(([key, value]) => {
            if (typeof value !== 'string' || !(0, compositeFunctionInterpolation_1.containsUnresolvedTemplateReference)(value)) {
                return [key, value];
            }
            return [key, (0, compositeFunctionInterpolation_1.stringifyInterpolatedResult)((0, compositeFunctionInterpolation_1.resolveInterpolatedTarget)(value, scoped))];
        }));
    ...
    resolveTemplatedWorkingDirectoryIfNeeded() {
        const scope = this.compositeFunctionScope;
        if (!scope) {
            return;
        }
        const relativeWorkingDirectory = this.ctx.relativeWorkingDirectory;
        if (relativeWorkingDirectory === undefined ||
            !(0, compositeFunctionInterpolation_1.containsUnresolvedTemplateReference)(relativeWorkingDirectory)) {
            return;
        }
        ...
        const resolved = (0, compositeFunctionInterpolation_1.resolveInterpolatedTarget)(relativeWorkingDirectory, context);
        this.ctx.updateRelativeWorkingDirectory((0, compositeFunctionInterpolation_1.stringifyOptionalInterpolatedResult)(resolved));
    }
}
