 ...
 * break its callers.
 */
async function constructHookEntriesAsync(ctx, steps, { externalFunctions, externalFunctionGroups, compositeFunctionCatalog, }) {
    ...
    return constructHookEntriesFromValidatedSteps(ctx, validatedSteps, new CompositeFunctionExpander_1.CompositeFunctionExpander(ctx, compositeFunctionCatalog ?? {}, {
        buildFunctionById,
...
function constructHookEntriesFromValidatedSteps(ctx, validatedSteps, compositeFunctionExpander) {
    const entries = [];
...
function createBuildStepFromShellStep(ctx, step) {
    ...
    const outputs = step.outputs && (0, step_1.createBuildStepOutputsFromDefinition)(ctx, step.outputs, displayName);
    return new BuildStep_1.BuildStep(ctx, {
        id: BuildStep_1.BuildStep.getNewId(step.id),
        displayName,
        outputs,
        workingDirectory: step.working_directory,
        shell: step.shell,
...
function validateAllStepFunctionsExist(steps, { externalFunctionIds, externalFunctionGroupIds, }) {
    ...
    for (const step of steps) {
        if (step.uses && !(0, localCompositeFunctions_1.isLocalCompositeFunctionPath)(step.uses)) {
