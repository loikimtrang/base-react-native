...
function validateAllFunctionsExist(config, { externalFunctionIds = [], externalFunctionGroupsIds = [], skipNamespacedFunctionsOrFunctionGroupsCheck, }) {
    ...
    const compositeFunctionPaths = calledFunctionsOrFunctionGroup.filter(localCompositeFunctions_1.isLocalCompositeFunctionPath);
    if (compositeFunctionPaths.length > 0) {
        throw new errors_1.BuildConfigError(`Local composite functions (${compositeFunctionPaths
            .map(compositeFunctionPath => `"${compositeFunctionPath}"`)
            .join(', ')}) are not supported in ".eas/build/*.yml" custom builds. Local composite functions can only be used in EAS workflows (".eas/workflows/*.yml").`);
    }
