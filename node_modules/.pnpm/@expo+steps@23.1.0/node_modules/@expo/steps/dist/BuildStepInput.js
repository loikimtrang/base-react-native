...
exports.getDisallowedInputValueError = getDisallowedInputValueError;
...
class BuildStepInput {
    ...
    getValue({ interpolationContext, skipLegacyOutputInterpolation = false, }) {
        ...
        if (valueDoesNotRequireInterpolation) {
            ...
            const valueInterpolatedWithOutputsAndGlobalContext = skipLegacyOutputInterpolation
                ...
                : (0, template_1.interpolateWithOutputs)(valueInterpolatedWithGlobalContext, path => this.ctx.getStepOutputValue(path) ?? '');
...
function getDisallowedInputValueError(input, stepDisplayName) {
    ...
    const rendered = typeof input.rawValue === 'object' ? JSON.stringify(input.rawValue) : String(input.rawValue);
